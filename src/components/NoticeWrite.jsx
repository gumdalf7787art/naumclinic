...
import { getNoticeById, addNotice, updateNotice } from '../utils/noticeStore';
...
const TEXT_COLORS = [
  ...
  '#EAB308', '#22C55E', '#0284C7', '#8B5CF6', '#EC4899', '#FFFFFF',
];
const BG_COLORS = [
...
export default function NoticeWrite() {
  ...
  useEffect(() => {
    if (isEditMode && editId) {
      const notice = getNoticeById(editId);
      if (notice) {
        setTitle(notice.title);
        setCategory(notice.category);
        setThumbnail(notice.image || '');
        if (editorRef.current) {
          editorRef.current.innerHTML = notice.content || '<p><br></p>';
        }
      }
    }
  }, [isEditMode, editId]);
  ...
  // Selection 복원 후 명령 실행
  const applyWithRestoredSelection = useCallback((fn) => {
    ...
    for (let r = 0; r < tableRows; r++) {
      ...
      for (let c = 0; c < tableCols; c++) {
        ...
        const style = r === 0
          ...
          : 'border:1px solid #cbd5e1;padding:8px 12px;text-align:center;min-width:60px;';
        html += `<${tag} style="${style}">${r === 0 ? `열 ${c + 1}` : '내용'}</${tag}>`;
      }
      html += '</tr>';
    }
    html += '</table><p><br></p>';
  ...
  const handleQuote = () => {
    ...
    document.execCommand('insertHTML', false,
      `<blockquote style="border-left:4px solid #0284c7;margin:12px 0;padding:10px 16px;background:#f0f9ff;color:#0369a1;border-radius:0 8px 8px 0;"><p>인용 내용을 입력하세요</p></blockquote><p><br></p>`
    );
  };
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        editorRef.current?.focus();
        document.execCommand('insertHTML', false,
          `<img src="${optimized}" style="max-width:100%;height:auto;border-radius:8px;margin:8px 0;display:block;" /><p><br></p>`
        );
      };
  ...
  const handlePublish = () => {
    ...
    if (isEditMode && editId) {
      const original = getNoticeById(editId);
      updateNotice({ ...original, title, category, content, image: thumbnail });
      alert('게시글이 수정되었습니다.');
      navigate(`/community/notice/${editId}`);
    } else {
      addNotice({
        id: Date.now().toString(), category, title, date,
        author: '관리자', image: thumbnail, hidden: false,
          ...
          <div className="bg-white rounded-xl p-6 shadow-2xl w-72">
            ...
            <div className="flex gap-4 mb-6">
              <div className="flex-1">
                <label className="text-xs text-gray-500 mb-1 block font-medium">행 (세로)</label>
              ...
              <div className="flex-1">
                <label className="text-xs text-gray-500 mb-1 block font-medium">열 (가로)</label>
      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-3xl font-bold text-gray-900 mb-1">
            {isEditMode ? '공지사항 수정' : '공지사항 글 작성'}
          </h2>
          <p className="text-gray-500 text-sm">{isEditMode ? '내용을 수정하고 저장하세요.' : '새로운 공지사항을 작성해 주세요.'}</p>
        </div>

        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-visible flex flex-col"
        >
          {/* ── TOOLBAR ── */}
          <div className="flex-shrink-0 bg-white border-b border-gray-100 rounded-t-2xl" style={{ position: 'sticky', top: 0, zIndex: 60 }}>
            {/* Row 1: Block elements */}
            <div className="flex items-center px-4 py-1.5 gap-0.5 border-b border-gray-100 overflow-x-auto bg-white rounded-t-2xl">
              <ToolbarBtn icon={<ImageIcon size={19} strokeWidth={1.5} />} label="사진" onClick={() => fileInputRef.current.click()} />
              <ToolbarBtn icon={<Quote size={19} strokeWidth={1.5} />} label="인용구" onClick={handleQuote} />
              <ToolbarBtn icon={<Minus size={19} strokeWidth={1.5} />} label="구분선" onClick={handleHR} />
              <ToolbarBtn icon={<LinkIcon size={19} strokeWidth={1.5} />} label="링크" onClick={handleLink} />
              <ToolbarBtn icon={<Table2 size={19} strokeWidth={1.5} />} label="표" onClick={() => setShowTableModal(true)} />
            </div>

            {/* Row 2: Text formatting */}
            <div className="flex items-center px-4 py-1.5 gap-1.5 overflow-x-auto bg-gray-50/60 flex-wrap">
              <select
                className="text-xs border border-gray-200 rounded px-2 py-1 bg-white text-gray-700 cursor-pointer outline-none"
                defaultValue="p"
              ...
              <FmtBtn label={<Bold size={14} />} onClick={() => exec('bold')} title="굵게" />
              <FmtBtn label={<Italic size={14} />} onClick={() => exec('italic')} title="기울임" />
              <FmtBtn label={<Underline size={14} />} onClick={() => exec('underline')} title="밑줄" />
              <FmtBtn label={<Strikethrough size={14} />} onClick={() => exec('strikeThrough')} title="취소선" />

              <div className="w-px h-4 bg-gray-300 mx-0.5" />

              <FmtBtn label={<AlignLeft size={14} />} onClick={() => exec('justifyLeft')} title="왼쪽 정렬" />
              <FmtBtn label={<AlignCenter size={14} />} onClick={() => exec('justifyCenter')} title="가운데 정렬" />
              <FmtBtn label={<AlignRight size={14} />} onClick={() => exec('justifyRight')} title="오른쪽 정렬" />
              <FmtBtn label={<List size={14} />} onClick={() => exec('insertUnorderedList')} title="목록" />
          ...
          <div
            ...
            className="px-10 py-10 text-gray-800 focus:outline-none outline-none"
            style={{ minHeight: '600px', lineHeight: 1.85 }}
          >
            <p><br /></p>
          </div>

          {/* ── FOOTER ── */}
          <div className="flex-shrink-0 px-8 py-5 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 rounded-b-2xl">
            <button
              onClick={() => navigate(-1)}
...
function ToolbarBtn({ icon, label, onClick }) {
  return (
    <button onClick={onClick}
      className="flex flex-col items-center justify-center w-[56px] h-[52px] rounded-lg hover:bg-gray-50 text-gray-600 transition-colors flex-shrink-0">
