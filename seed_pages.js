...
const pages = [
  ...
  },
  {
    ...
    menu_id: 11,
    title: '목·허리 척추 클리닉',
    subtitle: '거북목, 디스크 등 척추 질환 비수술 치료',
    content: `
...
for (const page of pages) {
  sql += "INSERT INTO subpages (slug, menu_id, title, subtitle, banner_image, content, is_published)\n" +
         "VALUES ('" + page.slug + "', " + page.menu_id + ", '" + page.title + "', '" + page.subtitle + "', '', '" + page.content + "', " + page.is_published + ")\n" +
         "ON CONFLICT(slug) DO UPDATE SET content = excluded.content, title = excluded.title, subtitle = excluded.subtitle;\n";
}

fs.writeFileSync('./migrations/seed_subpages.sql', sql);

console.log('Generated SQL file: ./migrations/seed_subpages.sql');
try {
  const result = execSync('npx wrangler d1 execute naum-db --local --file=./migrations/seed_subpages.sql');
